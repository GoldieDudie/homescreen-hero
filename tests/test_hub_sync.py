...
def test_move_to_top_anchors_after_recently_viewed_in_tv():
    from homescreen_hero.core.integrations import plex_client
    ...
    section._hubs[0].identifier = "tv.recentlyviewed"
    section._hubs[1].identifier = "tv.recentlyadded"
    section._hubs[2].identifier = "custom.collection.4.002"
    section._hubs[3].identifier = "custom.collection.4.003"

    server = FakeServer({"TV Series": section})

    # Pin to top: should land after the in-progress hub (tv.recentlyviewed),
    # which means ABOVE other built-ins like Recently Added.
    ...
    assert titles[2] == "Recently Added in TV Series"
